...
export function Sidebar({
  ...
  return (
    <aside
      className={cn(
        'relative z-10 flex h-full shrink-0 flex-col border-r border-line-soft bg-surface-2/75 backdrop-blur-2xl transition-[width] duration-300',
        collapsed ? 'w-16' : 'w-[252px]',
      )}
    >
      {/* Brand */}
      <div className={cn('pt-7 pb-5', collapsed ? 'px-2' : 'px-6')}>
        <div className={cn('flex items-center gap-3', collapsed && 'justify-center')}>
          <div className="scan-sheen relative grid h-11 w-11 shrink-0 place-items-center rounded-[12px] bg-gradient-to-br from-primary-3 via-primary to-primary-2 text-surface shadow-2 ring-1 ring-primary/50">
      ...
      <div className={cn('hairline', collapsed ? 'mx-2' : 'mx-5')} />

      {/* Nav */}
      <nav className={cn('stagger flex flex-1 flex-col gap-1.5 py-4', collapsed ? 'px-2' : 'px-3')}>
        <div
          className={cn(
        ...
        >
          {!collapsed && <span className="label-eyebrow">Navigation</span>}
          <button
            type="button"
            onClick={onToggle}
            title={collapsed ? '展开侧栏' : '收起侧栏'}
            className="grid h-7 w-7 shrink-0 place-items-center rounded-md border border-line-soft bg-surface/70 text-ink-faint transition-colors hover:border-line-strong hover:text-primary"
          >
            {collapsed ? (
